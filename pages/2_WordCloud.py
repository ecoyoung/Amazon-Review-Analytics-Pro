...
def load_negative_words():
    ...
    try:
        ...
        file_path = os.path.join(config_dir, 'negative_words.json')
        
        if os.path.exists(file_path):
            with open(file_path, 'r', encoding='utf-8') as f:
                return set(json.load(f))
    except Exception as e:
        st.warning(f"加载否定词文件时出错: {str(e)}")
...
def main():
    ...
    # 使用session state管理否定词
    if 'negative_words' not in st.session_state:
        st.session_state.negative_words = load_negative_words()
        ...
        try:
            ...
            try:
                if review_type == "Positive评论":
                    filtered_df = df[df['Review Type'].astype(str).str.lower() == 'positive']
                elif review_type == "Negative评论":
                    filtered_df = df[df['Review Type'].astype(str).str.lower() == 'negative']
                elif review_type == "Neutral评论":
                    filtered_df = df[df['Review Type'].astype(str).str.lower() == 'neutral']
                else:
                    filtered_df = df
            except Exception as e:
                st.warning(f"筛选数据时出现问题: {str(e)}，使用所有数据")
                filtered_df = df
            ...
            with st.container():
                ...
                with col1:
                    ...
                    if st.button("添加否定词", key="add_word") and new_negative_word:
                        ...
                        st.session_state.negative_words.update(words_to_add)
                        if save_negative_words(st.session_state.negative_words):
                            st.success(f"✅ 已添加 {len(words_to_add)} 个否定词")
                            st.rerun()
                
                with col2:
                    st.markdown("#### ❌ 删除否定词")
                    if st.session_state.negative_words:
                        word_to_remove = st.selectbox(
                            "选择要删除的否定词", 
                            sorted(list(st.session_state.negative_words)), 
                            key="remove_word"
                        )
                        if st.button("删除否定词", key="remove_btn"):
                            st.session_state.negative_words.remove(word_to_remove)
                            if save_negative_words(st.session_state.negative_words):
                ...
                st.markdown("#### 📦 预设否定词")
                # 直接使用字符串，避免字典键名问题
                preset_words_str = "supplements,supplement,gummy,gummies,capsule,capsules,drop,take,taking,took,also"
                
                col3, col4 = st.columns([3, 1])
                with col3:
                    st.markdown("预设否定词类别：保健品相关")
                    st.markdown("包含：supplements, supplement, gummy, gummies, capsule, capsules, drop, take, taking, took, also")
                with col4:
                    if st.button("一键导入预设否定词", key="import_preset"):
                        words_to_add = [word.strip().lower() for word in preset_words_str.split(',')]
                        st.session_state.negative_words.update(words_to_add)
                        if save_negative_words(st.session_state.negative_words):
                ...
                if st.session_state.negative_words:
                    # 使用英文逗号分隔，避免显示问题
                    words_display = ", ".join(sorted(st.session_state.negative_words))
                    st.markdown(f'<div class="negative-words-list">{words_display}</div>', 
                              unsafe_allow_html=True)
                else:
                    st.info("当前没有设置否定词")
            ...
            if st.button("☁️ 生成词云图", key="analyze", type="primary", use_container_width=True):
                ...
                with st.spinner('正在处理评论内容...'):
                    ...
                    for i, text in enumerate(filtered_df['Content']):
                        words = process_text(text, stop_words, st.session_state.negative_words)
                ...
                if word_freq:
                    ...
                    with st.container():
                        ...
                        wordcloud_fig, wordcloud = create_wordcloud(word_freq, st.session_state.negative_words)
                        if wordcloud_fig is not None:
                            st.pyplot(wordcloud_fig)
                            plt.close(wordcloud_fig)  # 释放内存
                        st.markdown('</div>', unsafe_allow_html=True)
                    ...
                    with col1:
                        ...
                        csv_data = word_freq_df.to_csv(index=False, encoding='utf-8')
                        st.download_button(
                            label="📊 下载词频统计CSV",
                            data=csv_data,
                            file_name=f"word_frequency_{review_type.replace('评论', '')}.csv",
                            mime="text/csv"
                        )
                        ...
                        if wordcloud is not None:
                            try:
                                ...
                                st.download_button(
                                    ...
                                    file_name=f"wordcloud_{review_type.replace('评论', '')}.png",
                                    mime="image/png"
                                )
                            except Exception as e:
